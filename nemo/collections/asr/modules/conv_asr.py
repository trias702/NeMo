...
__all__ = ['ConvASRDecoder', 'ConvASRSampledDecoder', 'ConvASREncoder', 'ConvASRDecoderClassification']
...
class ConvASRSampledDecoder(ConvASRDecoder):
    ...
    def __init__(self, feat_in, num_classes, n_samples, init_mode="xavier_uniform", vocabulary=None):
    ...
    @typecheck()
    def forward(self, encoder_output, transcript):
        if not torch.is_grad_enabled() or torch.is_inference_mode_enabled():
            # Simply call the parent
